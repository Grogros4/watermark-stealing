...
props = {
    "prf_type": (
        str
    ),  # string name of the underlying PRF mapping multiple token ids to a random seed
    "context_width": (
        int
    ),  # this is h in the paper, how many previous tokens should be considered for each PRF
    "self_salt": (
        bool
    ),  # Use the rules laid in robust-watermarking to use the token itself to seed and possibly reject its own list
    "hash_key": (
        int
    ),  # integer, large prime, used to move seed away from low-entrop bit sequences in PRF chosen above
}
...
def anchored_minhash_prf(input_ids: torch.LongTensor, salt_key: int, anchor: int = -1) -> int:
    arr = salt_key * hashint(input_ids) * hashint(input_ids[anchor])
    return arr.min().item()
...
def constant_prf(input_ids: torch.LongTensor, salt_key: int) -> int:
    return salt_key  # just plain and simple as sha is slow
